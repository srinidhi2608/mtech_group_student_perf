...
def preprocess(df):
    df = df.copy()
    # numeric features
    feature_cols = ['PUC_Score','Sem_1','Sem_2','Sem_3','Sem_4','Sem_5',
                    'Attendance_percent','Study_Hours_Per_Day','Sleep_Hours_Per_Day']
    df[feature_cols] = df[feature_cols].apply(pd.to_numeric, errors='coerce').fillna(0)

    # courses -> multi-hot
    df['Courses_Completed'] = df.get('Courses_Completed','').fillna('').astype(str)
    ...
    X = pd.concat([df[feature_cols].reset_index(drop=True), courses_df.reset_index(drop=True)], axis=1).fillna(0)
    y = pd.to_numeric(df.get('Sem_6', 0), errors='coerce').fillna(0)
    return X, y, mlb
...
def train(path="simple_student_dataset.csv", save_prefix="sem6_pred", test_size=0.2, random_state=42):
    ...
    if X.shape[0] < 10:
        print("Warning: small dataset; results may be noisy.")
